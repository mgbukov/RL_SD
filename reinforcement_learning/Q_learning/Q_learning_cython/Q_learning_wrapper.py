import numpy as _np
import struct
from .Q_learning import Q_learning_cython
...
class Q_learning():
	...
	def __init__(self,seed,model,params_QL,params_tabular):
		...
		Q_learning_cython(seed,model,params_QL,params_tabular,
							self.episodic_return,
							self.episodic_return_true,
							self.return_running_ave,
							self.return_true_ave,
							self.episodic_protocol,
							self.actions_best_encountered,
							self.actions_taken,
							self.return_best_encountered,
							self.states_best_encountered,
		)
